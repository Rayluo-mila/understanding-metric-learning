...
def worker_shared_memory_no_truncation_reset(
    ...
    def __init__(self, env_fns):
        ...
        for idx, env_fn in enumerate(self.env_fns):
            ...
            if self.single_action_space is None:
                ...
                assert (
                    act_space == self.single_action_space
                ), f"Action spaces differ between environments at index {idx}"

    def sample_actions(self):
        return np.array(
            [self.single_action_space.sample() for _ in range(self.num_envs)]
        )
...
def main():
    ...
    cfg = Config(**cfg_dict)
    env_list = [
        make_env(cfg, seed=cfg.seed + cfg.num_eval_episodes + 1)
