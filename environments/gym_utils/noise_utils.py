...
def recover_obs(projected_obs, inv_proj_matrix):
    ...
    original_obs = torch.matmul(
        torch.from_numpy(inv_proj_matrix), torch.from_numpy(projected_obs)
    ).numpy()
    return original_obs
...
def random_proj(obs, proj_matrix):
    ...
    """
    projected_obs = torch.matmul(
        torch.from_numpy(proj_matrix), torch.from_numpy(obs)
    ).numpy()
