...
class BaseVideoSource(ImageSource):
    def __init__(self, shape, filelist, total_frames=None, grayscale=False, random_frame=False, verbose=False):
        """
        Args:
            shape: [h, w]
    ...
    def build_arr(self):
        if not self.total_frames:
            ...
            for fname in tqdm.tqdm(self.filelist, desc=f"Loading video {fname}", position=0):
...
class RandomVideoSource(BaseVideoSource):
    def __init__(self, shape, filelist, total_frames=None, grayscale=False, random_frame=False, verbose=False):
        ...
        Args:
            ...
            filelist: a list of video files
        """
        super().__init__(shape, filelist, total_frames, grayscale, random_frame, verbose)
    ...
    def get_image(self, loc=None):
        ...
        else: img = self.arr[loc]
        if self.verbose: print(f'Current loc: {self._loc}, hash: {self.hash}')  # for debugging
        return img
