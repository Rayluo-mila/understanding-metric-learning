...
def add_noise(obs, loc=None, bg_source=None):
    ...
    if bg_source is not None:
        mask = np.logical_and((noised_obs[:, :, 2] > noised_obs[:, :, 1]), (noised_obs[:, :, 2] > noised_obs[:, :, 0]))  # hardcoded for dmc
...
class DMCWrapper(Env):
    ...
    def _define_spaces(self, from_pixels):
        ...
        self._state_observation_space = _spec_to_box(self._env.observation_spec().values())
    ...
    def _get_obs(self, time_step):
        if self._from_pixels:
            ...
            ).transpose(2, 0, 1)  # channel-last obs
    ...
    def step(self, action):
        if not self._norm_action_space.contains(action):
            print(f'action {action}, type {type(action)} NOT in norm action space {self._norm_action_space}')
