...
def make_agent(
    obs_shape, action_shape, actor_action_max, cfg, L, max_reward=None, min_reward=None
):
    """
    ...
    agent_configs = OmegaConf.merge(
        OmegaConf.load("cfgs/agent_configs.yaml"),
        OmegaConf.load("cfgs/agent_configs_exp.yaml"),
    )
    ...
    if "l1" in agent_name:
        if "mean" in cfg.bisim_dist:
            cfg.bisim_dist = "L1_mean"
        else:
            cfg.bisim_dist = "L1"
        cfg.r_dist = "L1"
    elif "l2" in agent_name:
        if "mean" in cfg.bisim_dist:
            cfg.bisim_dist = "L2_mean"
        else:
            cfg.bisim_dist = "L2"
        cfg.r_dist = "L2"
    ...
    if "norm" in cfg.encoder_post_processing and (
        cfg.encoder_type == "pixel"
        or cfg.encoder_type == "mlp"
        or cfg.encoder_type == "mlp_l2normed"
    ):
        post_processing_str = cfg.encoder_post_processing.replace("_", "")
        cfg.encoder_type += f"_{post_processing_str}ed"
    if "norm" in cfg.encoder_post_processing_metric and (
        cfg.encoder_type_metric == "pixel"
        or cfg.encoder_type_metric == "mlp"
        or cfg.encoder_type_metric == "mlp_l2normed"
    ):
...
class Timer:
    ...
    def __enter__(self):
        if not self.enabled_ctx:
            return self
        if self.active_timer is None:
            raise RuntimeError(
                "No timer name set. Use the 'time' method to set a timer name."
            )

        if self.active_timer not in self.timings:
            self.timings[self.active_timer] = {
                "start_time": None,
                "total_time": 0,
                "last_time_period": None,
            }
...
class BaseTrainer:
    def __init__(self, cfg):
        ...
        if cfg.agent.c_T is None:
            assert cfg.agent.c_R == 1.0
            self.cfg.agent.c_T = cfg.discount
        assert self.cfg.agent.c_R <= 1.0 and self.cfg.agent.c_T <= 1.0
        self.cfg.wandb_run_name = (
            None if cfg.wandb_run_name == "" else cfg.wandb_run_name
        )
        ...
        self.cfg.jobid = (
            str(os.environ["SLURM_JOB_ID"]) if "SLURM_JOB_ID" in os.environ else None
        )
        self.cfg.SLURM_ARRAY_TASK_ID = (
            str(os.environ["SLURM_ARRAY_TASK_ID"])
        ...
        self.L = Logger(
            cfg.work_dir,
            use_wandb=cfg.save_wandb,
            wandb_run_name=cfg.wandb_run_name,
            wandb_proj_name=cfg.wandb_proj_name,
            args=self.dict_cfg,
            wandb_freq=cfg.wandb_update_freq,
        )
    ...
    def calculate_num_updates(self):
        if (
            not hasattr(self, "update_counter")
            and self.global_step >= self.cfg.init_steps
        ):
    ...
    def eval_bisimilar_states(
        self, anchor, pos_samples, neg_samples, dist, encoder, compute_L2=True
    ):
        """
        Compute the similarity score between the anchor and the samples.
        ...
        z_anchor = self.get_representation(
            anchor, encoder
        )  # Shape: (batch_size_anchor, representation_dim)
        z_pos = self.get_representation(
            pos_samples_shaped, encoder
        )  # Shape: (batch_size_anchor * batch_size_pos, representation_dim)
        z_neg = self.get_representation(
            neg_samples_shaped, encoder
        )  # Shape: (batch_size_anchor * batch_size_neg, representation_dim)

        # Repeat z_anchor to match z_pos and z_neg
        z_anchor_pos = z_anchor.repeat_interleave(
            batch_size_pos, dim=0
        )  # Shape: (batch_size_anchor * batch_size_pos, representation_dim)
        z_anchor_neg = z_anchor.repeat_interleave(
            batch_size_neg, dim=0
        )  # Shape: (batch_size_anchor * batch_size_neg, representation_dim)

        # Compute pairwise distances and mean scores using metric_func
        _, pos_score = self._compute_sim_score(
            z_anchor_pos, z_pos, dist
        )  # Shape: (batch_size_anchor * batch_size_pos,)
        _, neg_score = self._compute_sim_score(
            z_anchor_neg, z_neg, dist
        )  # Shape: (batch_size_anchor * batch_size_neg,)
    ...
    def _record_and_report_performance(
        self, mean_eps_reward, mean_eps_reward_homo, eval_encoder_result
    ):
        perf_report_times = len(self.cfg.perf_report_start_step)
        if self.tabular_result_reported < perf_report_times:
            if (
                self.global_step
                >= self.cfg.perf_report_start_step[self.tabular_result_reported]
                and self.global_step
                <= self.cfg.perf_report_end_step[self.tabular_result_reported]
            ):
                self.perf_meter["n"] += 1
                ...
                if self.homo_eval_env:
                    self.perf_meter[
                        "episode_reward_homogeneous"
                    ] += mean_eps_reward_homo
            if (
                self.global_step
                >= self.cfg.perf_report_end_step[self.tabular_result_reported]
            ):
                ...
                self.log(
                    f"eval/tab_episode_reward_{log_step}",
                    self.perf_meter["episode_reward"] / self.perf_meter["n"],
                )
                if eval_encoder_result is not None:
                    self.log(
                ...
                if self.homo_eval_env:
                    self.log(
                        f"eval_debug/tab_episode_reward_homogeneous_{log_step}",
                        self.perf_meter["episode_reward_homogeneous"]
                        / self.perf_meter["n"],
                    )
    ...
    def terminate(self):
        ...
        print(
            f"Taking {self.timer.get_global_elapsed_time() / 3600} hrs. Terminating..."
        )
