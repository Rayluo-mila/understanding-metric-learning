...
from environments.gym_utils.wrappers import (
    PhysicalStateWrapper,
    GaussianNoiseWrapper,
    GaussianRandomProjectionWrapper,
)
...
from environments.gym_utils.noise_utils import (
    recover_obs,
    random_proj,
    append_white_noise,
...
def make_env(
    cfg,
    seed,
    bg_source="unspecified",
    is_eval=True,
    proj_matrix=None,
    inv_proj_matrix=None,
):
    env_ = dmc2gym.make(
    ...
    if cfg.noise_source == "noise":
        ...
        env_ = GaussianRandomProjectionWrapper(
            env_, cfg.noise_dim, cfg.noise_std, proj_matrix, inv_proj_matrix, noise_mean
        )
    return env_
...
def make_env_fn(
    ...
    return functools.partial(
        make_env, cfg, seed, bg_source, is_eval, proj_matrix, inv_proj_matrix
    )
...
class DMCStateTrainer(BaseTrainer):
    ...
    def init(self, cfg):
        ...
        if cfg.noise_source == "random_proj":
            ...
            matrix_n = (
                matrix_m if cfg.random_proj_dim == "identical" else cfg.random_proj_dim
            )
            print(
                f"Generating random projection matrix with shape {matrix_n} * {matrix_m}..."
            )
            self.proj_matrix = generate_full_rank_matrix(
                matrix_n, matrix_m, cfg.random_proj_std
            )
        ...
        # Print info
        print(
            f"Obs dim: {self.true_obs_dim}, Physical state dim: {self.physical_state_dim}"
        )
        print(
            f"Distractor: {cfg.noise_source}, Noise dim: {cfg.noise_dim}, std: {cfg.noise_std}"
        )
    ...
    def eval(self, embed_viz_dir=None):
        eval_bism_states = (
            ...
            and (self.cfg.noise_source in ["noise", "color", "random_proj"])
        )
    ...
    def eval_encoder(self):
        ...
        for i in range(total_samples):
            ...
            _positive_obses = [
                self._add_noise_to_obs(clean_obs) for _ in range(pos_neg_batch_size)
            ]
            _positive_train_obses = [
                self._add_noise_to_obs(clean_obs, is_ood=False)
        ...
        if self.cfg.noise_source == "noise":
            if not is_ood:
                noised_obs = append_white_noise(
                    clean_obs, self.cfg.noise_dim, self.cfg.noise_std
                )
            else:
                noised_obs = append_white_noise(
                    clean_obs,
