...
class DMCPixelTrainer(BaseTrainer):
    ...
    def init(self, cfg):
        ...
        if not cfg.use_vectorized_training_env:
            ...
            elif cfg.training_env_homogeneous_level == 0:  # Heterogeneous noise sources for each environment
                self.env = AsyncVectorEnv([make_env_fn(
                    cfg, seed=cfg.seed+i, is_eval=False
                    ) for i in range(cfg.num_parallel_envs)], worker=worker_shared_memory_no_truncation_reset)
                train_bg_source = deepcopy(self.env.call(name='get_bg_source')[0])
            elif cfg.training_env_homogeneous_level == 1:  # Uniform noise sources across all parallel environments
                self.individual_homo_train_env = make_env(cfg, seed=cfg.seed, is_eval=False)
                train_bg_source = deepcopy(self.individual_homo_train_env.unwrapped.get_bg_source())
                self.env = AsyncVectorEnv([make_env_fn(
                    cfg, seed=cfg.seed+i, is_eval=False, bg_source=deepcopy(train_bg_source)
                    ) for i in range(cfg.num_parallel_envs)], worker=worker_shared_memory_no_truncation_reset)
                self.env.call(name='reset_bg_source')
                train_bg_source = deepcopy(self.env.call(name='get_bg_source')[0])
            elif cfg.training_env_homogeneous_level == 2:  # Synchronized noise backgrounds for all parallel environments
                self.individual_homo_train_env = make_env(cfg, seed=cfg.seed, is_eval=False)
                train_bg_source = deepcopy(self.individual_homo_train_env.unwrapped.get_bg_source())
                self.env = AsyncVectorEnv([make_env_fn(
                    cfg, seed=cfg.seed+i, is_eval=False, bg_source=deepcopy(train_bg_source)
        ...
        self.eval_env = AsyncVectorEnv([make_env_fn(
            cfg, seed=cfg.seed+cfg.num_parallel_envs+1+i
        ...
        self.individual_homo_eval_env = make_env(cfg, seed=cfg.seed+cfg.num_parallel_envs+1)
    ...
    def train(self):
        ...
        for self._global_step in range(0, self.cfg.num_train_steps, self.cfg.num_parallel_envs):
            if truncated:
                ...
                if self.cfg.save_bg_debug: self.save_dmc_obs(obs, 'reset')
                truncated = False
                episode_reward = 0
                episode_step = 0
            ...
            obs = next_obs
            if self.cfg.save_bg_debug: self.save_dmc_obs(obs, 'next')
    ...
    def eval(self, embed_viz_dir=None):
        ...
        if self.cfg.save_bg_debug: self.save_dmc_obs(obs, 'eval_reset')
        ...
        while not truncated:
            ...
            if self.cfg.save_bg_debug: self.save_dmc_obs(obs, f'eval_next_st{episode_step}')
            if self.homo_eval_env:
                obs_homo, reward_homo, _, _, _ = self.homo_eval_env.step(action_homo)
                if self.cfg.save_bg_debug: self.save_dmc_obs(obs_homo, f'eval_homo_next_st{episode_step}')
            if self.eval_train_env: 
                obs_train, reward_train, _, _, _ = self.eval_train_env.step(action_train)
                if self.cfg.save_bg_debug: self.save_dmc_obs(obs_train, f'eval_train_next_st{episode_step}')
    ...
    def eval_encoder(self):
        ...
        for i in range(total_samples):
            ...
            for _ in range(pos_neg_batch_size):
                ...
                pos_obs = self._add_noise_to_obs(clean_obs, self.eval_env_noise_srcs[rnd_idx])
                pos_train_obs = self._add_noise_to_obs(clean_obs, self.eval_train_env_noise_srcs[rnd_idx])
    ...
    def _add_noise_to_obs(self, clean_obs, noise_source, noise_vid_loc=None):
        ...
        """
        # Precompute slices
        slices = [(self.cfg.frame_stack * j, self.cfg.frame_stack * (j + 1)) for j in range(self.cfg.frame_stack)]
        if noise_vid_loc:
            new_loc = (noise_vid_loc + np.random.randint(
                self.cfg.frame_stack, self.cfg.eval_total_frames)) % self.cfg.eval_total_frames
        else:
            new_loc = (np.random.randint(
                self.cfg.frame_stack, self.cfg.eval_total_frames)) % self.cfg.eval_total_frames
        noise_offsets = [new_loc - self.cfg.frame_stack + 1 + j for j in range(self.cfg.frame_stack)]

        # Generate another_noised_obs using list comprehension and vectorized operations
        noised_obs = np.concatenate(
            [add_noise(clean_obs[st:ed, :, :], offset, noise_source) for (st, ed), offset in zip(slices, noise_offsets)],
            axis=0
        )
        return noised_obs
    ...
    def _save_dmc_obs(self, obs, filename_suffix=''):
        ...
        grid_img.save(dir / f'ep{self.global_episode}_st{self.global_step}_{filename_suffix}.png')
        print(f'Saved observation grid image to {dir / f"ep{self.global_episode}_st{self.global_step}_{filename_suffix}.png"}')
