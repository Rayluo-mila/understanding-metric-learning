...
def get_max_norm(metric_ub, cfg, metric_encoder=False):
    ...
    if (cfg.encoder_max_norm and not metric_encoder) or (
        ...
        if "L1" in cfg.bisim_dist:
            encoder_max_norm_ord = 1
        elif "huber" in cfg.bisim_dist:
            encoder_max_norm_ord = 1
        elif "L2" in cfg.bisim_dist:
            encoder_max_norm_ord = 2
        else:
            encoder_max_norm_ord = 2
...
class SACAgent(object):
    def __init__(self, obs_shape, action_shape, actor_action_max, cfg, L):
        ...
        self.encoder_max_norm, self.encoder_max_norm_ord = get_max_norm(
            self.metric_ub, cfg
        )
    ...
    def update_critic(self, obs, action, reward, next_obs, not_done, step):
        ...
        current_Q1, current_Q2, h_norm = self.critic(
            obs, action, detach_encoder=self.cfg.no_phi_q_star, require_hnorm=True
        )
        ...
        if self.cfg.report_grad_norm:
            ...
            self.grad_norm_analysis(
                ...
                critic_parameters,
                step,
                label="critic_all_loss",
                post_backward=True,
            )
        if self.cfg.clip_grad_norm_critic > 0:
            nn.utils.clip_grad.clip_grad_norm_(
                self.critic.parameters(), self.cfg.clip_grad_norm_critic
            )
    ...
    def update_actor_and_alpha(self, obs, step):
        ...
        if self.cfg.clip_grad_norm_actor > 0:
            nn.utils.clip_grad.clip_grad_norm_(
                self.actor.parameters(), self.cfg.clip_grad_norm_actor
            )
    ...
    def grad_norm_analysis(self, loss, parameters, step, label="", post_backward=False):
        ...
        if not post_backward:
            if (
                isinstance(loss, float) or isinstance(loss, int)
            ) and loss == 0:  # loss is zero
                return
            ...
            grads_norms = [
                torch.linalg.norm(g.detach().data).item() ** 2
                for g in grads
                if g is not None
            ]
        else:
            grads_norms = [
                torch.linalg.norm(p.grad.detach().data).item() ** 2
                for p in parameters
    ...
    def metric_func(self, x, y, step=None):
        z_dist = metric_func(
            x, y, self.cfg, self.metric_ub, step, self.L
        )  # (batch_size)
        return z_dist

    def prob_metric_func(self, mu, mu2, sigma, sigma2):
        zprime_dist = prob_metric_func(
            mu, mu2, sigma, sigma2, self.cfg, metric_ub=self.metric_ub
        )  # (batch_size)
        return zprime_dist
