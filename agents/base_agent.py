...
class TD3Agent(object):
    def __init__(self, obs_shape, action_shape, actor_action_max, cfg, L):
        ...
        self.critic_tau = cfg.critic_tau
        self.encoder_tau = cfg.encoder_tau
        self.policy_delay = cfg.policy_delay  # TD3 uses delayed policy updates
        ...
        self.action_max = actor_action_max
    ...
    def select_action(self, obs, **kwargs):
        multiproc = len(obs.shape) == 4
        with torch.no_grad():
            obs = torch.FloatTensor(obs).to(self.device)
            obs = obs.unsqueeze(0) if not multiproc else obs
            mu = self.actor(obs)
            if not multiproc:
                return mu.cpu().data.numpy().flatten()
            else:
    ...
    def sample_action(self, obs, multiproc=False, **kwargs):
        # For TD3, sampling action is the same as selecting action
        return self.select_action(obs, multiproc=multiproc, **kwargs)

    def update_critic(self, obs, action, reward, next_obs, not_done, step):
        with torch.no_grad():
            # Add target policy smoothing
            noise = (torch.randn_like(action) * self.policy_noise).clamp(-self.noise_clip, self.noise_clip)
            next_action = (self.actor_target(next_obs) + noise).clamp(-self.action_max, self.action_max)
    ...
    def update_actor(self, obs, step):
        ...
        if step % self.policy_delay == 0:
            ...
            self.soft_update_params(self.actor, self.actor_target, self.encoder_tau)
    ...
    def grad_norm_analysis(self, loss, parameters, step, label='', post_backward=False):
        if step % 100 != 0: return
        if not post_backward:
            if (isinstance(loss, float) or isinstance(loss, int)) and loss == 0:
                return
            grads = torch.autograd.grad(
                outputs=loss, inputs=parameters, create_graph=False, retain_graph=True, allow_unused=True
            )
