...
class RAPAgent(BisimAgent):
    ...
    def __init__(self, obs_shape, action_shape, actor_action_max, cfg, L):
        ...
        self.state_reward_decoder = StateRewardDecoder(cfg.encoder_feature_dim).to(
            self.device
        )
    ...
    def update_encoder(self, obs, next_obs, action, reward, step):
        ...
        if self.cfg.state_rp_detach_encoder:
            ...
            reward_mu, reward_sigma = self.state_reward_decoder(z)
        state_reward_decoder_loss = self.state_reward_decoder.loss(
            reward_mu, reward_sigma, reward
        )
        r_var = reward_sigma.detach().pow(2.0)
        if self.cfg.bisim_coef <= 0:
            return 0, state_reward_decoder_loss
        ...
        # Compute metric loss
        diff_square = (z_dist - self.cfg.c_T * transition_dist).pow(2.0)
        loss = F.smooth_l1_loss(diff_square, self.cfg.c_R * r_dist, reduction="mean")

        # Stats
        if self.cfg.r_dist == "rap":
            bisimilarity = (
                self.cfg.c_R * torch.sqrt(torch.relu(r_dist))
                + self.cfg.c_T * transition_dist
            )
        else:
    ...
    def update(self, batch_transition, step, **kwargs):
        ...
        if (
            ...
            encoder_loss, state_reward_decoder_loss = self.update_encoder(
                obs_op, next_obs_op, action_op, reward_op, step
            )
        else:
            encoder_loss, state_reward_decoder_loss = self.update_encoder(
                obs, next_obs, action, reward, step
            )
        total_loss = (
            bisim_coef * encoder_loss
        ...
        if self.cfg.report_grad_norm:
            ...
            self.grad_norm_analysis(
                ...
                encoder_parameters,
                step,
                label="state_reward_decoder_loss",
            )
