        ...
        with torch.no_grad():
            ...
            if self.sample_next_state_from_buffer:
                ...
                pred_z_prime = self.transition_model.sample_prediction(
                    torch.cat([z_target, action], dim=1))  # shape (B, Z)

        z, z2, reward, reward2, pred_z_prime, pred_z_prime2, _, _ = self.generate_bisim_samples(
            z, z_target, reward, pred_z_prime, None)
