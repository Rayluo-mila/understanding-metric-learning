...
def metric_func(x, y, cfg, metric_ub=None, step=None, L=None, override=None):
    ...
    if bisim_dist_type == "L1":
        ...
        base_distances = torch.atan2(
            _sqrt(1.0 - cos_similarity.pow(2.0)), cos_similarity
        )
        norm_average = x.pow(2.0).sum(dim=-1) + y.pow(2.0).sum(dim=-1)
...
def prob_metric_func(mu, mu2, sigma, sigma2, cfg, metric_ub=None):
    if cfg.prob_dist == "":
        if "mean" in cfg.bisim_dist:
            dist = torch.sqrt((mu - mu2).pow(2) + (sigma - sigma2).pow(2)).mean(dim=-1)
        else:
        ...
        if "L2_mean" in cfg.prob_dist:
            if "nostd" in cfg.prob_dist:
                ...
                dist = torch.sqrt((mu - mu2).pow(2) + (sigma - sigma2).pow(2)).mean(
                    dim=-1
                )
            ...
            if "nostd" in cfg.prob_dist:
                sigma_dist = torch.tensor(0.0, device=mu.device)
            else:
            ...
            dist = torch.tensor(0.0, device=mu.device)
        else:
            raise NotImplementedError
    if metric_ub is not None:
        dist = torch.clamp(dist, max=metric_ub, min=0.0)
    return dist
