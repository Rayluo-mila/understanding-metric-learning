...
class IsolatedMetricSACAgent(BisimAgent):
    ...
    def __init__(self, obs_shape, action_shape, actor_action_max, cfg, L):
        ...
        encoder_max_norm, encoder_max_norm_ord = get_max_norm(
            self.metric_ub, cfg, metric_encoder=True
        )
        self.encoder_metric = make_encoder(
            cfg.encoder_type_metric,
            obs_shape,
            cfg.metric_encoder_feature_dim,
            cfg.num_layers,
            cfg.num_filters,
            cfg.encoder_stride,
        ...
        self.transition_model = make_transition_model(
            ...
            encoder_max_norm_ord=self.encoder_max_norm_ord,
        ).to(cfg.device)
        if self.cfg.rp_zprime:
            self.reward_decoder = nn.Sequential(
                nn.Linear(
                    cfg.metric_encoder_feature_dim, cfg.reward_decoder_hidden_size
                ),
            ...
            self.reward_decoder = nn.Sequential(
                nn.Linear(
                    cfg.metric_encoder_feature_dim + action_shape[0],
        ...
        self.metric_optimizer = torch.optim.Adam(
            self.encoder_metric.parameters(), lr=cfg.metric_encoder_lr
        )
    ...
    def print_model_stats(self):
        ...
        print(
            f"Metric Encoder params count: {rl_utils.count_parameters(self.encoder_metric)}"
        )
    ...
    def update_encoder(self, obs, next_obs, action, reward, step):
        ...
        with torch.no_grad():
            ...
            if self.transition_model_type in ["", "deterministic", "ensemble_det"]:
                ...
                pred_z_prime_mu = self.transition_model.sample_prediction(
                    torch.cat([z_target, action], dim=1)
                )
            else:
                pred_z_prime_mu, pred_z_prime_sigma = self.transition_model(
                    torch.cat([z_target, action], dim=1)
                )
            ...
            if self.cfg.model_based_on_policy_metric_update:
                if self.cfg.rp_zprime:
                    ...
                    reward_on_policy = self.reward_decoder(
                        torch.cat([z, action], dim=1)
                    )
                self.log(
                    "train_metric/reward_buffer_on_policy_gap",
                    (reward - reward_on_policy).abs().mean(),
                    step,
                )
        ...
        if "det" in self.transition_model_type or self.transition_model_type == "":
            ...
            if step % 500 == 0:
                self.log(
                    "train_metric/z_prime_mu_l2norm",
                    pred_z_prime_mu.norm(p=2, dim=-1).mean(),
                    step,
                )
                self.log(
                    "train_metric/z_prime_sigma_l2norm",
                    pred_z_prime_sigma.norm(p=2, dim=-1).mean(),
                    step,
                )
    ...
    def update_transition_reward_model(self, obs, action, next_obs, reward, step):
        ...
        if self.need_zp:
            ...
            if self.cfg.metric_zp_detach_encoder:
                z_zp = z_zp.detach()
            pred_z_prime_mu, pred_z_prime_sigma = self.transition_model(
                torch.cat([z_zp, action], dim=1)
            )
            ...
            transition_loss = torch.mean(
                0.5 * diff.pow(2) + torch.log(pred_z_prime_sigma)
            )
        ...
        if self.need_rp:
            ...
            if self.cfg.metric_rp_detach_encoder:
                z_rp = z_rp.detach()
            if self.cfg.rp_zprime:
                pred_z_prime = self.transition_model.sample_prediction(
                    torch.cat([z_rp, action], dim=1)
                )
    ...
    def update(self, batch_transition, step, **kwargs):
        ...
        if (
            "on_policy_transition" in kwargs
            and kwargs["on_policy_transition"] is not None
        ):
        ...
        if self.cfg.bisim_coef <= 0:
            ...
            encoder_loss = self.update_encoder(
                obs_op, next_obs_op, action_op, reward_op, step
            )
        else:
        ...
        if self.cfg.report_grad_norm:
            metric_parameters = [
                p for p in self.encoder_metric.parameters() if p.requires_grad
            ]
            self.grad_norm_analysis(
                encoder_loss, metric_parameters, step, label="metric_loss"
            )
            encoder_parameters = [
                p for p in self.encoder_metric.parameters() if p.requires_grad
            ]
            self.grad_norm_analysis(
                ...
                encoder_parameters,
                step,
                label="transition_reward_loss",
            )

        self.decoder_optimizer.zero_grad()
        self.metric_optimizer.zero_grad()
        if transition_reward_loss != 0:
    ...
    def soft_update_params(self, step):
        ...
        if step % self.critic_target_update_freq == 0:
            rl_utils.soft_update_params(
                self.encoder_metric, self.encoder_metric_target, self.encoder_tau
            )

    def save(self, model_dir, step):
        super().save(model_dir, step)
        torch.save(
            self.encoder_metric.state_dict(),
            "%s/encoder_metric_%s.pt" % (model_dir, step),
        )
        torch.save(
            self.encoder_metric_target.state_dict(),
            "%s/encoder_metric_target_%s.pt" % (model_dir, step),
        )
