...
class BasePixelEncoder(nn.Module):
    ...
    def compute_output_dim(self, h, w):
        for conv in self.convs:
            ...
            h = (
                h - kernel_size_h
            ) // stride_h + 1  # ceil((h - kernel_size + 2 * padding) / stride)
...
class PixelEncoder(BasePixelEncoder):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__(
            obs_shape, feature_dim, num_layers, num_filters, stride, **kwargs
        )
    ...
    def forward(self, obs, detach=False):
        ...
        if self.max_norm:
            norm_to_max = (
                (torch.linalg.norm(out, ord=self.max_norm_ord, dim=-1) / self.max_norm)
                .clamp(min=1)
                .unsqueeze(-1)
            )
            out = out / norm_to_max
