...
class BisimAgent(SACAgent):
    ...
    def log_metric_stats(
        ...
        with torch.no_grad():
            ...
            self.log(
                "train_metric/mu_rd_div_bd", self.mu_rd / (self.mu_bd + 1e-9), step
            )
    ...
    def generate_bisim_samples(self, z, z_target, reward, mu, sigma, r_var=None):
        ...
        if mu.ndim == 2:  # Shape (B, Z), no ensemble
            ...
            sigma1, sigma2 = (
                (sigma[idx1], sigma[idx2]) if sigma is not None else (None, None)
            )
    ...
    def update(self, batch_transition, step, **kwargs):
        ...
        total_loss = (
            bisim_coef * encoder_loss + self.cfg.rzp_coef * transition_reward_loss
        )
        if self.cfg.report_grad_norm:
            ...
            self.grad_norm_analysis(
                bisim_coef * encoder_loss, encoder_parameters, step, label="metric_loss"
            )
            self.grad_norm_analysis(
                self.cfg.rzp_coef * transition_reward_loss,
                encoder_parameters,
                step,
