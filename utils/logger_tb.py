...
FORMAT_CONFIG = {
    'rl': {
        'train': [
            ...
            ('q_loss', 'QLOSS', 'float'),
            ('critic_loss', 'CLOSS', 'float'), ('metric_loss', 'MLOSS', 'float'),
...
class Logger(object):
    def __init__(self, log_dir, use_tb=True, tb_name='tb', config='rl', args=None):
        self._log_dir = log_dir
        if use_tb:
            # tb_name += '_' + time.strftime("%Y%m%d%H%M%S",time.localtime())
            tb_dir = os.path.join(log_dir, tb_name)
        ...
        if args is not None and self._sw is not None:
            self._sw.add_hparams(args, {'null': 0})
