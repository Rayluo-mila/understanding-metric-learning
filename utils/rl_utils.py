...
def set_seed_everywhere(seed):
    ...
    # os.environ['CUBLAS_WORKSPACE_CONFIG'] = ":4096:8"  # TODO: added for the next line, may remove
...
class ReplayBufferDiscrete(object):
    """Buffer to store environment transitions."""
    def __init__(self, obs_shape, action_shape, capacity, batch_size, device, nprocs=1):
        print(f'Building replay buffer with capacity={capacity}, B={batch_size}, obs={obs_shape}')
        self.capacity = capacity
        ...
        self.k_obses = np.empty((capacity, *obs_shape), dtype=obs_dtype)
        self.next_obses = np.empty((capacity, *obs_shape), dtype=obs_dtype)
        self.actions = np.empty((capacity, 1), dtype=np.int64)  # discrete
        self.curr_rewards = np.empty((capacity, 1), dtype=np.float32)
        self.rewards = np.empty((capacity, 1), dtype=np.float32)
        self.not_dones = np.empty((capacity, 1), dtype=np.float32)
    ...
    def add_multi(self, obs, action, curr_reward, reward, next_obs, done):
        np.copyto(self.obses[self.idx:self.idx+self.nprocs], obs)
        np.copyto(self.actions[self.idx:self.idx+self.nprocs], action)
        np.copyto(self.curr_rewards[self.idx:self.idx+self.nprocs], np.expand_dims(curr_reward, axis=-1))
        np.copyto(self.rewards[self.idx:self.idx+self.nprocs], np.expand_dims(reward, axis=-1))
        np.copyto(self.next_obses[self.idx:self.idx+self.nprocs], next_obs)
        np.copyto(self.not_dones[self.idx:self.idx+self.nprocs], np.expand_dims([not _ for _ in done], axis=-1))

        self.idx = (self.idx + self.nprocs) % self.capacity
        self.full = self.full or self.idx == 0
    ...
    def sample_recent(self, k=False):
        """
        Get the most recent batch_size samples
        """
        idxs = np.array([(self.capacity + self.idx - 1 - i) % self.capacity for i in range(self.batch_size)])
