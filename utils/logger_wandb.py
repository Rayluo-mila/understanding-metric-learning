...
class Logger(object):
    def __init__(self, log_dir, use_wandb=True,
                 wandb_proj_name='Metrics', wandb_run_name=None,
                 config='rl', args=None, wandb_freq=1):
    ...
    def dump(self, step):
        if self._dump_count % self._dump_freq == 0:
            for key in self._use_wandb_params:
                value = self.get(key)
                if value is not None: self._try_wandb_log(key, value, step)
